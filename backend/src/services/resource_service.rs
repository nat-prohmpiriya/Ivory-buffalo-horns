...
impl ResourceService {
    ...
    pub async fn update_village_resources(pool: &PgPool, village_id: Uuid) -> AppResult<Village> {
        ...
        let crop_produced = (production.crop_per_hour as f64 * hours_elapsed) as i32;

        // Calculate new resource amounts (capped at storage)
        let new_wood = (village.wood + wood_produced).min(village.warehouse_capacity);
        let new_clay = (village.clay + clay_produced).min(village.warehouse_capacity);
        let new_iron = (village.iron + iron_produced).min(village.warehouse_capacity);
        let new_crop = (village.crop + crop_produced).min(village.granary_capacity);
